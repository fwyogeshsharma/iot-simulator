        ...
        private Object generateValue(DataTypeConfig config) {
            if ("enum".equals(config.getConfigType())) {
                ...
                if ("blood_pressure".equals(config.getDataType())) {
                    ...
                    result.put("diastolic", diastolicMin + new Random().nextInt(diastolicMax - diastolicMin + 1));
                    return result;
                } else if ("gps".equals(conf.get("type"))) {
                    // Special handling for GPS location data (identified by "type": "gps" in config)
                    Map<String, Double> latRange = (Map<String, Double>) conf.get("latitude");
                    Map<String, Double> lonRange = (Map<String, Double>) conf.get("longitude");

                    double latMin = latRange != null ? latRange.getOrDefault("min", -90.0) : -90.0;
                    double latMax = latRange != null ? latRange.getOrDefault("max", 90.0) : 90.0;
                    double lonMin = lonRange != null ? lonRange.getOrDefault("min", -180.0) : -180.0;
                    double lonMax = lonRange != null ? lonRange.getOrDefault("max", 180.0) : 180.0;
